...
public class KnnPattern
{
	...
	public static class KnnMapper extends Mapper<Object, Text, NullWritable, DoubleString>
	{
		...
		double normalisedDeceased_date;
		String Sex;
		String City;
		double normalisedConfirmed_date;
		
		// The known ranges of the dataset, which can be hardcoded in for the purposes of this example
		double minAge = 18;
		double maxAge = 99;
		double minDeceased_date = 1;
		double maxDeceased_date = 13;
		double minConfirmed_date = 0;
		double maxConfirmed_date = 7;
		...
		{	
			...
			double Deceased_dateDifference = S2 - R2;
			double sexDifference = nominalDistance(S3, R3);
			double cityDifference = nominalDistance(S4, R4);
			double Confirmed_dateDifference = S5 - R5;
			...
			return squaredDistance(ageDifference) + squaredDistance(Deceased_dateDifference) + sexDifference + cityDifference + squaredDistance(Confirmed_dateDifference);
		}
		...
		protected void setup(Context context) throws IOException, InterruptedException
		{
			if (context.getCacheFiles() != null && context.getCacheFiles().length > 0)
			{
				...
				normalisedDeceased_date = normalisedDouble(st.nextToken(), minDeceased_date, maxDeceased_date);
				Sex = st.nextToken();
				City = st.nextToken();
				normalisedConfirmed_date = normalisedDouble(st.nextToken(), minConfirmed_date, maxConfirmed_date);
			}
		}
		...
		public void map(Object key, Text value, Context context) throws IOException, InterruptedException
		{
			...
			double normalisedRDeceased_date = normalisedDouble(st.nextToken(), minDeceased_date, maxDeceased_date);
			String rSex = st.nextToken();
			String rCity = st.nextToken();
			double normalisedRConfirmed_date = normalisedDouble(st.nextToken(), minConfirmed_date, maxConfirmed_date);
			...
			double tDist = totalSquaredDistance(normalisedRAge, normalisedRDeceased_date, rSex, rCity,
					normalisedRConfirmed_date, normalisedSAge, normalisedSIncome, sStatus, sGender, normalisedSChildren);
